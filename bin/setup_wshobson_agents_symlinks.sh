...
# Script to create symlinks for agent files from wshobson-agents folder to ~/.claude/agents folder
...
SOURCE_DIR="$PROJECT_ROOT/agents/wshobson-agents"

# Check if source directory exists
if [ ! -d "$SOURCE_DIR" ]; then
    echo -e "${RED}ERROR:${NC} Directory $SOURCE_DIR does not exist. Please run install.sh first."
    exit 1
fi
...
echo "Setting up wshobson agent symlinks to ~/.claude/agents..."
echo

# Work with absolute paths
current_dir="$PROJECT_ROOT"

# Loop through all files in wshobson-agents directory
for file in "$SOURCE_DIR"/*; do
    ...
    # Target symlink path with wshobson prefix
    target_path="$HOME/.claude/agents/wshobson-$file"

    # Check if symlink or file already exists
    if [ -L "$target_path" ] || [ -e "$target_path" ]; then
        skipped_files+=("wshobson-$file")
        echo -e "${YELLOW}SKIPPED:${NC} wshobson-$file (already exists)"
    else
        # Create symlink
        ln -s "$SOURCE_DIR/$file" "$target_path"
        if [ $? -eq 0 ]; then
            echo -e "${GREEN}LINKED:${NC}  wshobson-$file -> ~/.claude/agents/wshobson-$file"
        else
            echo -e "${RED}ERROR:${NC}   Failed to create symlink for wshobson-$file"
            skipped_files+=("wshobson-$file")
        fi
    fi
done
...
echo "Your wshobson agent files are now symlinked to ~/.claude/agents/"
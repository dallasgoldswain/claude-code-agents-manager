...
# Script to remove dLabs agent symlinks from ~/.claude/agents/
...
echo -e "${YELLOW}Removing dLabs agent symlinks...${NC}"
echo
...
# Remove dLabs agent symlinks
for link_path in "$HOME/.claude/agents"/dLabs-*; do
    # Check if the glob pattern matched any files
    if [ ! -e "$link_path" ]; then
        echo -e "${YELLOW}No dLabs agent symlinks found.${NC}"
        break
    fi
...
if [ $removed_count -gt 0 ]; then
    echo -e "${GREEN}dLabs agent symlinks removal complete!${NC}"
else
    echo -e "${YELLOW}No dLabs agent symlinks were found to remove.${NC}"
fi